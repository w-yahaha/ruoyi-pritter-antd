...
export default () => ({
  tableItem: [
    ...
    { prop: 'todo', label: '操作', width: 200, slotName: 'todo', fixed: 'right' },
  ],
  tableConfig: {
    rowKey: 'postId',
...
const [dialogRef, infoInit, addClick, editBtnClick] = useDialog(null, null, '添加')
...
<template>
  <PageSearch ref="pageSearchRef" :pageName="pageName" :searchConfig="searchConfig" />
  ...
  <PageContent
    ...
    :permission="{ add: 'system:post:add', edit: 'system:post:edit', del: 'system:post:remove' }"
...
export default () => ({
  ...
  formItems: [
    { field: 'postName', type: 'input', label: '岗位名称', config: { maxlength: 30 } },
    { field: 'postSort', type: 'inputNumber', label: '岗位顺序', config: { min: 0 } },
    { field: 'status', type: 'radio', label: '状态', isGroup: true, options: [] },
  ],
  colLayout: { span: 24 },
  formConfig: {
    layout: 'horizontal',
